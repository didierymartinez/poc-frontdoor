...
import type { VistaAnticipo } from '@/entities/borrador';

function getColumns(): GridColDef<VistaAnticipo>[] {
  return [
    {
      field: 'terceroNombre',
      headerName: 'Tercero',
...
interface AnticiposBorradoresPanelProps {
  rows: VistaAnticipo[];
  isPending: boolean;
}

export function AnticiposBorradoresPanel({ rows, isPending }: AnticiposBorradoresPanelProps) {
  ...
  const filtered = rows.filter((r) => {
    if (search) {
      return [r.terceroNombre, r.terceroIdentificacion, r.id].some((v) =>
  ...
  if (rows.length === 0) {
    return <EmptyState title="Sin borradores" description="No hay anticipos pendientes de confirmación." />;
  }
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flex: 1, minHeight: 0 }}>
        {filtered.length === 0 ? (
          <EmptyState title="Sin resultados" description="No se encontraron anticipos con los filtros aplicados." />
        ) : (
          <DataGridPro
            rows={filtered}
            columns={columns}
            ...
            onRowClick={(params) => {
              navigate(`/registro-anticipo/${params.row.id}`, { state: { fromOcr: true } });
            }}
            sx={{
              border: 'none',
